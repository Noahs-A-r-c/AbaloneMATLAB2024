% MagIndNex3.m is a next coordinate search function that does not account
% for rotation by only taking the surrounding magnitude directions. it uses
% vector subtraction.
function [indexXNext, indexYNext] = MagIndNext3(BxByCat, indexXFirst, indexYFirst, secondBx, secondBy,surroundRange)
    ...
    surroundingArrayDiff = surroundingArray - repmat(catt,size(surroundingArray,1),1);
